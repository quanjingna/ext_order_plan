...
def back_testing(index, lines):
    ...
    rs = back_valid.back_testing(loop=False,use_pool=False).tail(30)
...
if __name__ == '__main__':
    ...
    brdcst_forecasting_periods = sc.broadcast(30)
    ...
    #spark.sql(
    #    "insert overwrite table app.app_vsc_ord_forecast_seller_output_backtest_test partition("
    #    "dt='" + forecast_date + "')  select * from data_rs_combined "
    #)
    spark.sql("set hive.exec.dynamic.partition.mode=nonstrict")
    spark.sql("set hive.exec.dynamic.partition=true")
    spark.conf.set("spark.sql.sources.partitionOverwriteMode", "dynamic")
    data_rs_combined.select("forecast_dt", "region_id", "region_name", "seller_no", "seller_name", "dept_no","dept_name", "wh_store_no", "wh_store_name", "forecast_value", "real_value", "bias", "dt").coalesce(1).write.mode("overwrite").format("Hive").insertInto("app.app_vsc_ord_forecast_seller_output_backtest_30",overwrite=True)
