...
def forecast_test(index, lines):
    ...
    lines = raw_data_process.compute(use_pool=False)
    if lines.shape[0]==0:
        return iter([])
    batch = SimpleStrategies(lines, freq='d', forecast_periods=90)
    rs = batch.compute(use_pool=False).tail(90)
    # 转化为Row list
    def get_row(line):
        d = line.to_dict()
        for name in [Y_HAT]:
...
if __name__ == '__main__':
    ...
    #print(obj_size)
    forecast_date = str(pd.to_datetime(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())[0:10]) - datetime.timedelta(1))[0:10]
    forecast_datetime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
    #forecast_date = "2019-05-22"
    print(forecast_date)
    ...
    col_datatype = [StructField(OBJ_NO, IntegerType(), False)] + \
                   [StructField(name, DoubleType(), False) for name in [Y_HAT]] + \
    ...
    # forecast_df.persist(StorageLevel.MEMORY_AND_DISK)
    # forecast_df.createOrReplaceTempView("data_rs")
    # 存储
    # sql_str = "select split(obj_str, '_')[0] as seller_no, split(obj_str, '_')[1] as dept_no, split(obj_str, '_')[3] as sku_no, split(obj_str, '_')[2] as wh_store_no, ds as forecast_dt, yhat as forecast_value, '" + forecast_datetime + "' as create_time from data_rs"
    sql = "select	tt.forecast_dt,	null as region_id,	tt.region_name,	tt.seller_no,	c.seller_name,	tt.dept_no,	c.dept_name,	tt.warehouse_no as wh_store_no,	c.warehouse_name as wh_store_name,	round(tt.forecast_value *(		case			when tt.rate is null			then 0			else tt.rate		end), 0) as forecast_value from	(		select			a.forecast_dt,			a.region_name,			a.seller_no,			a.dept_no,			b.warehouse_no,			b.rate,			a.forecast_value		from			(				select					ds as forecast_dt,					split(obj_str, '_') [0] as region_name,					split(obj_str, '_') [1] as seller_no,					split(obj_str, '_') [2] as dept_no,					yhat as forecast_value				from					data_rs			)			a		left join			(				select * from rate_data			)			b		on			a.region_name = b.region_name			and a.seller_no = b.seller_no			and a.dept_no = b.dept_no	)	tt left join	(		select			region_name,			seller_no,			seller_name,			dept_no,			dept_name,			warehouse_no,			warehouse_name		from			wh_data		group by			region_name,			seller_no,			seller_name,			dept_no,			dept_name,			warehouse_no,			warehouse_name	)	c on	tt.warehouse_no = c.warehouse_no	and tt.region_name = c.region_name	and tt.seller_no = c.seller_no	and tt.dept_no = c.dept_no"
    ...
    spark.sql(
        "insert overwrite table app.app_vsc_ord_forecast_seller_output partition("
